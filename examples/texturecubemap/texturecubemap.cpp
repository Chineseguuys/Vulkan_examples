...
public:
    ...
    struct UniformData {
        ...
        float lodBias = 0.0f;
        // add for light control
        int useAmbientLight = 1;
        int useDiffuseLight = 1;
        int useSpecularLight = 1;
    } uniformData;
    ...
    virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay)
    {
        if (overlay->header("Settings")) {
            ...
            // add for light control
            overlay->checkBox("ambient light", &uniformData.useAmbientLight);
            overlay->checkBox("diffuse light", &uniformData.useDiffuseLight);
            overlay->checkBox("specular light", &uniformData.useSpecularLight);
        }
    }
};
