...
/*
 * Summary:
 * Using push constants it's possible to pass a small bit of static data to a shader, which is stored in the command
 * buffer stat This is perfect for passing e.g. static per-object data or parameters without the need for descriptor
 * sets The sample uses these to push different static parameters for rendering multiple objects
 */
...
public:
        ...
        for (auto i = 0; i < uniformBuffers.size(); i++) {
            ...
            std::vector<VkWriteDescriptorSet> writeDescriptorSets = {
                vks::initializers::writeDescriptorSet(
                    descriptorSets[i], VK_DESCRIPTOR_TYPE_UNIFORM_BUFFER, 0, &uniformBuffers[i].descriptor),
            };
    ...
    void preparePipelines() {
        ...
        // Note that the spec only requires a minimum of 128 bytes, so for passing larger blocks of data you'd use UBOs
        // or SSBOs
        VkPushConstantRange pushConstantRange{};
        // Push constants will only be accessible at the selected pipeline stages, for this sample it's the vertex
        ...
        shaderStages[0] =
            loadShader(getShadersPath() + "pushconstants/pushconstants.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
        shaderStages[1] =
            loadShader(getShadersPath() + "pushconstants/pushconstants.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
#ifdef DEBUG
        spdlog::info("[{}:{}]Use Vertex   Spriv: {}", __FILE__, __LINE__, getShadersPath() + "pushconstants/pushconstants.vert.spv");
        spdlog::info("[{}:{}]Use Fragment Spriv: {}", __FILE__, __LINE__, getShadersPath() + "pushconstants/pushconstants.frag.spv");
#endif // DEBUG
    ...
    void buildCommandBuffer() {
        ...
        for (uint32_t j = 0; j < spherecount; j++) {
            ...
            vkCmdPushConstants(
                cmdBuffer, pipelineLayout, VK_SHADER_STAGE_VERTEX_BIT, 0, sizeof(SpherePushConstantData), &spheres[j]);
