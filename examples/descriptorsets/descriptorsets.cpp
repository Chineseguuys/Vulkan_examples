...
public:
    ...
    ~VulkanExample() {
        ...
        for (auto &cube : cubes) {
            ...
            for (auto &buffer : cube.uniformBuffers) { buffer.destroy(); }
        }
    }

    virtual void getEnabledFeatures() {
        if (deviceFeatures.samplerAnisotropy) { enabledFeatures.samplerAnisotropy = VK_TRUE; };
    }
    ...
    void loadAssets() {
        ...
        cubes[0].texture.loadFromFile(
            getAssetPath() + "textures/crate01_color_height_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue);
        cubes[1].texture.loadFromFile(
            getAssetPath() + "textures/crate02_color_height_rgba.ktx", VK_FORMAT_R8G8B8A8_UNORM, vulkanDevice, queue);
    }
    ...
    */
    void setupDescriptors() {
        ...
        */
        ...
        // Uniform buffers are duplicated, so we can update buffer n+1 on the CPU while the GPU still reads from buffer
        // n
        ...
        // Images are static, and never update after initial upload, but as they use the same descriptor set as buffers,
        ...
        // Max. number of descriptor sets that can be allocated from this pool (one per object and per max. frame in
        // flight)
        ...
        /*
            ...
            The layout describes the shader bindings and types used for a certain descriptor layout and as such must
           match the shader bindings
        ...
        */
        ...
        // Accessible from the vertex shader only (flags can be combined to make it accessible to multiple shader
        // stages)
        ...
        */
        ...
        for (auto &cube : cubes) {
            // We have one uniform buffer per cube and max. fame in flight, so we also need one descriptor per that
            // setup
    ...
    void prepareUniformBuffers() {
        ...
        // Another option instead of using separate buffers could be using one large buffer with separate ranges per
        // cube and fram
        for (auto &cube : cubes) {
    ...
    void updateUniformBuffers() {
        ...
        for (auto &cube : cubes) {
            ...
            cube.matrices.model =
                glm::rotate(cube.matrices.model, glm::radians(cube.rotation.x), glm::vec3(1.0f, 0.0f, 0.0f));
            cube.matrices.model =
                glm::rotate(cube.matrices.model, glm::radians(cube.rotation.y), glm::vec3(0.0f, 1.0f, 0.0f));
            cube.matrices.model =
                glm::rotate(cube.matrices.model, glm::radians(cube.rotation.z), glm::vec3(0.0f, 0.0f, 1.0f));
    ...
    void buildCommandBuffer() {
        ...
        */
        for (auto cube : cubes) {
            // Bind the cube's descriptor set. This tells the command buffer to use the uniform buffer for the current
            // frame and image set for this cube
    ...
    virtual void render() {
        ...
        if (animate && !paused) {
            ...
            if (cubes[0].rotation.x > 360.0f) cubes[0].rotation.x -= 360.0f;
            cubes[1].rotation.y += 2.0f * frameTimer;
            if (cubes[1].rotation.y > 360.0f) cubes[1].rotation.y -= 360.0f;
        }
    ...
    virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
        if (overlay->header("Settings")) { overlay->checkBox("Animate", &animate); }
    }
};
