 ...
 *
 * This sample shows how to setup multiple graphics pipelines and how to use them for drawing objects with differring
 * visuals
 *
...
public:
    ...
    ~VulkanExample() {
        if (device) {
            ...
            if (enabledFeatures.fillModeNonSolid) { vkDestroyPipeline(device, pipelines.wireframe, nullptr); }
    ...
    virtual void getEnabledFeatures() {
        ...
        if (deviceFeatures.wideLines) { enabledFeatures.wideLines = VK_TRUE; }
    }
    ...
    void preparePipelines() {
        ...
        if (enabledFeatures.fillModeNonSolid) {
            ...
            shaderStages[1] =
                loadShader(getShadersPath() + "pipelines/wireframe.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
            VK_CHECK_RESULT(
    ...
    void updateUniformBuffers() {
        ...
        TRACE_EVENT("vulkan_example", "updateUniformBuffers");
#endif
    ...
    void buildCommandBuffer() {
#if defined(PERFETTO_SYSTRACE_PRINT)
        TRACE_EVENT("vulkan_example", "buildCommandBuffer");
#endif
        ...
        if (enabledFeatures.wideLines) { vkCmdSetLineWidth(cmdBuffer, 2.0f); }
        scene.draw(cmdBuffer);
    ...
    virtual void OnUpdateUIOverlay(vks::UIOverlay *overlay) {
        if (!enabledFeatures.fillModeNonSolid) {
            if (overlay->header("Info")) { overlay->text("Non solid fill modes not supported!"); }
        }
    }
};
