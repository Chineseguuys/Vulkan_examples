...
public:
    ...
    ~VulkanExample() {
        if (device) {
            if (uboDataDynamic.model) { alignedFree(uboDataDynamic.model); }
    ...
    void preparePipelines() {
        ...
        std::vector<VkVertexInputAttributeDescription> vertexInputAttributes = {
            vks::initializers::vertexInputAttributeDescription(
                0, 0, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Vertex, pos)), // Location 0 : Position
            vks::initializers::vertexInputAttributeDescription(
                0, 1, VK_FORMAT_R32G32B32_SFLOAT, offsetof(Vertex, color)), // Location 1 : Color
        };
        ...
        shaderStages[0] =
            loadShader(getShadersPath() + "dynamicuniformbuffer/base.vert.spv", VK_SHADER_STAGE_VERTEX_BIT);
        shaderStages[1] =
            loadShader(getShadersPath() + "dynamicuniformbuffer/base.frag.spv", VK_SHADER_STAGE_FRAGMENT_BIT);
    ...
    void prepareUniformBuffers() {
...
#ifdef DEBUG
        spdlog::info("[{}:{}]minUboAlignment = {}", __FILE__, __LINE__, minUboAlignment);
#endif // DEBUG
        dynamicAlignment = sizeof(glm::mat4);
        if (minUboAlignment > 0) {
            dynamicAlignment = (dynamicAlignment + minUboAlignment - 1) & ~(minUboAlignment - 1);
#ifdef DEBUG
            spdlog::info("[{}:{}]dynamicAlignment = {}", __FILE__, __LINE__, dynamicAlignment);
#endif // DEBUG
        }
        ...
        for (auto &buffer : uniformBuffers) {
            ...
            VK_CHECK_RESULT(vulkanDevice->createBuffer(
                VK_BUFFER_USAGE_UNIFORM_BUFFER_BIT, VK_MEMORY_PROPERTY_HOST_VISIBLE_BIT, &buffer.dynamic, bufferSize));
    ...
    void updateDynamicUniformBuffer() {
        ...
        memcpy(uniformBuffers[currentBuffer].dynamic.mapped,
    ...
    void buildCommandBuffer() {
        ...
        for (uint32_t j = 0; j < OBJECT_INSTANCES; j++) {
            ...
            vkCmdBindDescriptorSets(
                cmdBuffer, VK_PIPELINE_BIND_POINT_GRAPHICS, pipelineLayout, 0, 1, &descriptorSet, 1, &dynamicOffset);
    ...
    // render() 每帧流程:
    // ┌─ prepareFrame():
    // │   vkWaitForFences(waitFences[currentBuffer])  ← 等待当前帧的 GPU 工作完成
    // │   vkResetFences(waitFences[currentBuffer])
    // │   swapChain.acquireNextImage(presentCompleteSemaphores[currentBuffer], currentImageIndex)
    // │
    // ├─ updateUniformBuffers()       → memcpy 到 uniformBuffers[currentBuffer]
    // ├─ updateDynamicUniformBuffer() → memcpy 到 uniformBuffers[currentBuffer]
    // ├─ buildCommandBuffer()         → 录制到 drawCmdBuffers[currentBuffer]
    // │
    // └─ submitFrame():
    //     vkQueueSubmit(..., waitFences[currentBuffer])
    //     vkQueuePresentKHR(...)
    //     currentBuffer = (currentBuffer + 1) % 2   ← 这里！切换到下一帧索引
    virtual void render() {
        if (!prepared) return;
