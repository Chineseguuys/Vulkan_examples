...
VkResult VulkanExampleBase::createInstance() {
    ...
    // Shaders generated by Slang require a certain SPIR-V environment that can't be satisfied by Vulkan 1.0, so we need
    // to expliclity up that to at least 1.1 and enable some required extensions
    if (shaderDir == "slang") {
        if (apiVersion < VK_API_VERSION_1_1) { apiVersion = VK_API_VERSION_1_1; }
    ...
    // SRS - When running on iOS/macOS with MoltenVK and VK_KHR_portability_enumeration is defined and supported by the
    // instance, enable the extension and the flag
    ...
    if (std::find(
            supportedInstanceExtensions.begin(), supportedInstanceExtensions.end(), VK_EXT_DEBUG_UTILS_EXTENSION_NAME)
        != supportedInstanceExtensions.end()) {
...
std::string VulkanExampleBase::getWindowTitle() const {
    ...
    if (!settings.overlay) { windowTitle += " - " + std::to_string(frameCounter) + " fps"; }
    return windowTitle;
}
...
//     - for macOS, handle benchmarking within NSApp rendering loop via displayLinkOutputCb()
    ...
    if (benchmark.active) {
        ...
        if (wl_display_dispatch_pending(display) == -1) return;
#endif
        benchmark.run([=, this] { render(); }, vulkanDevice->properties);
        vkDeviceWaitIdle(device);
        if (!benchmark.filename.empty()) { benchmark.saveResults(); }
        return;
    }
#endif
    ...
    while (!quitMessageReceived) {
        ...
        if (prepared && !IsIconic(window)) { nextFrame(); }
    }
#elif defined(VK_USE_PLATFORM_ANDROID_KHR)
    while (true) {
        int ident;
        ...
        while ((ident = ALooper_pollOnce(focused ? 0 : -1, nullptr, &events, (void **)&source))
        ...
        if (prepared) {
            ...
            if (touchDown) { touchTimer += frameTimer; }
            if (touchTimer >= 1.0) { camera.keys.up = true; }
    ...
    while (!quit) {
        ...
        while (!event_buffer->GetEvent(event_buffer, DFB_EVENT(&event))) { handleEvent(&event); }
        render();
        frameCounter++;
        auto tEnd = std::chrono::high_resolution_clock::now();
        auto tDiff = std::chrono::duration<double, std::milli>(tEnd - tStart).count();
...
#if defined(VK_USE_PLATFORM_ANDROID_KHR)
    if (mouseState.buttons.left) { mouseState.buttons.left = false; }
#endif
}
...
#if defined(PERFETTO_SYSTRACE_PRINT)
    TRACE_EVENT("vulkan_example", "prepareFrame");
#endif
    if (waitForFence) {
...
#if defined(PERFETTO_SYSTRACE_PRINT)
    TRACE_EVENT("vulkan_example", "submitFrame");
#endif
    if (!skipQueueSubmit) {
...
VulkanExampleBase::VulkanExampleBase() {
    ...
    commandLineParser.add(
        "validationlogfile", {"-vl", "--validationlogfile"}, 0, "Log validation messages to a textfile");
    ...
    commandLineParser.add(
        "benchmarkwarmup", {"-bw", "--benchwarmup"}, 1, "Set warmup time for benchmark mode in seconds");
    commandLineParser.add(
        "benchmarkruntime", {"-br", "--benchruntime"}, 1, "Set duration time for benchmark mode in seconds");
    commandLineParser.add("benchmarkresultfile", {"-bf", "--benchfilename"}, 1, "Set file name for benchmark results");
    commandLineParser.add(
        "benchmarkresultframes", {"-bt", "--benchframetimes"}, 0, "Save frame times to benchmark results file");
    commandLineParser.add(
        "benchmarkframes", {"-bfs", "--benchmarkframes"}, 1, "Only render the given number of frames");
#if (!(defined(VK_USE_PLATFORM_IOS_MVK) || defined(VK_USE_PLATFORM_MACOS_MVK) || defined(VK_USE_PLATFORM_METAL_EXT)))
    commandLineParser.add(
        "resourcepath", {"-rp", "--resourcepath"}, 1, "Set path for dir where assets and shaders folder is present");
#endif
    ...
    if (commandLineParser.isSet("validation")) { settings.validation = true; }
    if (commandLineParser.isSet("validationlogfile")) { vks::debug::logToFile = true; }
    if (commandLineParser.isSet("vsync")) { settings.vsync = true; }
    if (commandLineParser.isSet("height")) { height = commandLineParser.getValueAsInt("height", height); }
    if (commandLineParser.isSet("width")) { width = commandLineParser.getValueAsInt("width", width); }
    if (commandLineParser.isSet("fullscreen")) { settings.fullscreen = true; }
    ...
    if (commandLineParser.isSet("benchmarkresultframes")) { benchmark.outputFrameTimes = true; }
        ...
        if (vkEnumerateInstanceExtensionProperties(nullptr, &extCount, &extensions.front()) == VK_SUCCESS) {
            for (VkExtensionProperties &extension : extensions) {
                if (std::strcmp(extension.extensionName, VK_EXT_LAYER_SETTINGS_EXTENSION_NAME) == 0) {
                    ...
                    // Configure MoltenVK to use to use a dedicated compute queue (see compute[*] and timelinesemaphore
                    // samples)
                    VkLayerSettingEXT layerSetting{.pLayerName = "MoltenVK",
    ...
    if (this->settings.validation) { setupConsole("Vulkan example"); }
    setupDPIAwareness();
#endif
}
...
VulkanExampleBase::~VulkanExampleBase() {
    ...
    if (settings.overlay) { ui.freeResources(); }
    delete vulkanDevice;
    if (settings.validation) { vks::debug::freeDebugCallback(instance); }
...
bool VulkanExampleBase::initVulkan() {
    ...
    if (commandLineParser.isSet("validationlogfile")) { vks::debug::log("Sample: " + title); }
    ...
    if (settings.validation) { vks::debug::setupDebugging(instance); }
...
void VulkanExampleBase::setupDPIAwareness() {
    ...
    if (shCore) {
        ...
        if (setProcessDpiAwareness != nullptr) { setProcessDpiAwareness(PROCESS_PER_MONITOR_DPI_AWARE); }
...
void VulkanExampleBase::handleMessages(HWND hWnd, UINT uMsg, WPARAM wParam, LPARAM lParam) {
    ...
    case WM_KEYDOWN:
        ...
        case KEY_F1: ui.visible = !ui.visible; break;
        case KEY_F2:
    ...
    case WM_MBUTTONDOWN:
        ...
        break;
    case WM_LBUTTONUP: mouseState.buttons.left = false; break;
    case WM_RBUTTONUP: mouseState.buttons.right = false; break;
    case WM_MBUTTONUP: mouseState.buttons.middle = false; break;
    case WM_MOUSEWHEEL: {
...
int32_t VulkanExampleBase::handleAppInput(struct android_app *app, AInputEvent *event) {
        ...
        case AINPUT_SOURCE_TOUCHSCREEN: {
            ...
            case AMOTION_EVENT_ACTION_UP: {
                ...
                if (eventTime - downTime <= vks::android::TAP_TIMEOUT) {
                    ...
                    if ((x * x + y * y) < deadZone) { vulkanExample->mouseState.buttons.left = true; }
                };
            ...
            case AMOTION_EVENT_ACTION_MOVE: {
                ...
                if (!handled) {
                    ...
                    float deltaX =
                        (float)(vulkanExample->touchPos.y - eventY) * vulkanExample->camera.rotationSpeed * 0.5f;
                    float deltaY =
                        (float)(vulkanExample->touchPos.x - eventX) * vulkanExample->camera.rotationSpeed * 0.5f;
    ...
    if (AInputEvent_getType(event) == AINPUT_EVENT_TYPE_KEY) {
        ...
        case AKEYCODE_F1:
        case AKEYCODE_BUTTON_L1: vulkanExample->ui.visible = !vulkanExample->ui.visible; break;
...
void VulkanExampleBase::handleAppCommand(android_app *app, int32_t cmd) {
    ...
    case APP_CMD_TERM_WINDOW:
        ...
        if (vulkanExample->prepared) { vulkanExample->swapChain.cleanup(); }
        break;
    }
}
...
- (void)applicationDidFinishLaunching:(NSNotification *)aNotification {
    ...
    dispatch_group_async(concurrentGroup, concurrentQueue, ^{
      while (!vulkanExample->quit) { vulkanExample->displayLinkOutputCb(); }
    });
...
- (void)keyDown:(NSEvent *)event {
    ...
    case KEY_P: vulkanExample->paused = !vulkanExample->paused; break;
    case KEY_1: // support keyboards with no function keys
    case KEY_F1: vulkanExample->ui.visible = !vulkanExample->ui.visible; break;
    case KEY_DELETE: // support keyboards with no escape key
    case KEY_ESCAPE: [NSApp terminate:nil]; break;
    case KEY_W: vulkanExample->camera.keys.up = true; break;
    case KEY_S: vulkanExample->camera.keys.down = true; break;
    case KEY_A: vulkanExample->camera.keys.left = true; break;
    case KEY_D: vulkanExample->camera.keys.right = true; break;
    default:
...
- (void)keyUp:(NSEvent *)event {
    switch (event.keyCode) {
    case KEY_W: vulkanExample->camera.keys.up = false; break;
    case KEY_S: vulkanExample->camera.keys.down = false; break;
    case KEY_A: vulkanExample->camera.keys.left = false; break;
    case KEY_D: vulkanExample->camera.keys.right = false; break;
    default: break;
    }
}
...
- (void)scrollWheel:(NSEvent *)event {
    ...
    vulkanExample->camera.translate(
        glm::vec3(0.0f, 0.0f, -(float)wheelDelta * 0.05f * vulkanExample->camera.movementSpeed));
}
...
void *VulkanExampleBase::setupWindow(void *view) {
    ...
    if (settings.fullscreen) { [window toggleFullScreen:nil]; }
...
void VulkanExampleBase::displayLinkOutputCb() {
    ...
    if (benchmark.active) {
        ...
        if (benchmark.filename != "") { benchmark.saveResults(); }
...
IDirectFBSurface *VulkanExampleBase::setupWindow() {
    ...
    if (settings.fullscreen) {
        ...
        if (layer_config.width > width) posx = (layer_config.width - width) / 2;
        if (layer_config.height > height) posy = (layer_config.height - height) / 2;
    }
...
void VulkanExampleBase::handleEvent(const DFBWindowEvent *event) {
    switch (event->type) {
    case DWET_CLOSE: quit = true; break;
    case DWET_MOTION: handleMouseMove(event->x, event->y); break;
    case DWET_BUTTONDOWN:
        switch (event->button) {
        case DIBI_LEFT: mouseState.buttons.left = true; break;
        case DIBI_MIDDLE: mouseState.buttons.middle = true; break;
        case DIBI_RIGHT: mouseState.buttons.right = true; break;
        default: break;
        }
        break;
    case DWET_BUTTONUP:
        switch (event->button) {
        case DIBI_LEFT: mouseState.buttons.left = false; break;
        case DIBI_MIDDLE: mouseState.buttons.middle = false; break;
        case DIBI_RIGHT: mouseState.buttons.right = false; break;
        default: break;
        }
        break;
    case DWET_KEYDOWN:
        ...
        case KEY_D: camera.keys.right = true; break;
        case KEY_P: paused = !paused; break;
        case KEY_F1: ui.visible = !ui.visible;
        default: break;
        }
        break;
    case DWET_KEYUP:
...
/*static*/ void
VulkanExampleBase::pointerLeaveCb(void *data, wl_pointer *pointer, uint32_t serial, wl_surface *surface) {
}

/*static*/ void
VulkanExampleBase::pointerMotionCb(void *data, wl_pointer *pointer, uint32_t time, wl_fixed_t sx, wl_fixed_t sy) {
    VulkanExampleBase *self = reinterpret_cast<VulkanExampleBase *>(data);
...
void VulkanExampleBase::pointerButton(struct wl_pointer *pointer,
                                      ...
                                      uint32_t state) {
    switch (button) {
    case BTN_LEFT: mouseState.buttons.left = !!state; break;
    case BTN_MIDDLE: mouseState.buttons.middle = !!state; break;
    case BTN_RIGHT: mouseState.buttons.right = !!state; break;
    default: break;
    }
}

/*static*/ void
VulkanExampleBase::pointerAxisCb(void *data, wl_pointer *pointer, uint32_t time, uint32_t axis, wl_fixed_t value) {
    VulkanExampleBase *self = reinterpret_cast<VulkanExampleBase *>(data);
...
void VulkanExampleBase::pointerAxis(wl_pointer *pointer, uint32_t time, uint32_t axis, wl_fixed_t value) {
    ...
    switch (axis) {
    case REL_X: camera.translate(glm::vec3(0.0f, 0.0f, d * 0.005f)); break;
    default: break;
    }
}

/*static*/ void
VulkanExampleBase::keyboardKeymapCb(void *data, struct wl_keyboard *keyboard, uint32_t format, int fd, uint32_t size) {
}
...
void VulkanExampleBase::keyboardKey(struct wl_keyboard *keyboard,
    ...
    case KEY_P:
        ...
        break;
    case KEY_F1:
        if (state) { ui.visible = !ui.visible; }
        break;
    case KEY_ESCAPE: quit = true; break;
    }
...
struct xdg_surface *VulkanExampleBase::setupWindow() {
    ...
    std::string windowTitle = getWindowTitle();
    xdg_toplevel_set_title(xdg_toplevel, windowTitle.c_str());
    if (settings.fullscreen) { xdg_toplevel_set_fullscreen(xdg_toplevel, NULL); }
...
xcb_window_t VulkanExampleBase::setupWindow() {
    ...
    xcb_change_property(
        connection, XCB_PROP_MODE_REPLACE, window, (*reply).atom, 4, 32, 1, &(*atom_wm_delete_window).atom);
...
void VulkanExampleBase::handleEvent(const xcb_generic_event_t *event) {
    ...
    case XCB_CLIENT_MESSAGE:
        if ((*(xcb_client_message_event_t *)event).data.data32[0] == (*atom_wm_delete_window).atom) { quit = true; }
        break;
    ...
    case XCB_BUTTON_PRESS: {
        ...
        if (press->detail == XCB_BUTTON_INDEX_1) mouseState.buttons.left = true;
        if (press->detail == XCB_BUTTON_INDEX_2) mouseState.buttons.middle = true;
        if (press->detail == XCB_BUTTON_INDEX_3) mouseState.buttons.right = true;
    } break;
    case XCB_BUTTON_RELEASE: {
        xcb_button_press_event_t *press = (xcb_button_press_event_t *)event;
        if (press->detail == XCB_BUTTON_INDEX_1) mouseState.buttons.left = false;
        if (press->detail == XCB_BUTTON_INDEX_2) mouseState.buttons.middle = false;
    ...
    case XCB_CONFIGURE_NOTIFY: {
        ...
        if ((prepared) && ((cfgEvent->width != width) || (cfgEvent->height != height))) {
            ...
            if ((destWidth > 0) && (destHeight > 0)) { windowResize(); }
        }
    } break;
    default: break;
    }
    ...
    while (!screen_get_event(screen_context, screen_event, paused ? ~0 : 0)) {
        ...
        case SCREEN_EVENT_KEYBOARD:
            ...
            if ((keyflags & KEY_SYM_VALID) == KEY_SYM_VALID) {
                switch (val) {
                case KEYCODE_ESCAPE: quit = true; break;
                case KEYCODE_W:
                ...
                case KEYCODE_D:
                    ...
                    break;
                case KEYCODE_P: paused = !paused; break;
                case KEYCODE_F1: ui.visible = !ui.visible; break;
                default: break;
                }

                if ((keyflags & KEY_DOWN) == KEY_DOWN) {
                    if ((val >= 0x20) && (val <= 0xFF)) { keyPressed(val); }
                }
            }
            break;
        ...
        case SCREEN_EVENT_POINTER:
            ...
            if ((mouse_buttons & SCREEN_LEFT_MOUSE_BUTTON) == 0) {
                if ((val & SCREEN_LEFT_MOUSE_BUTTON) == SCREEN_LEFT_MOUSE_BUTTON) { mouseState.buttons.left = true; }
            } else {
                if ((val & SCREEN_LEFT_MOUSE_BUTTON) == 0) { mouseState.buttons.left = false; }
            }
            if ((mouse_buttons & SCREEN_RIGHT_MOUSE_BUTTON) == 0) {
                if ((val & SCREEN_RIGHT_MOUSE_BUTTON) == SCREEN_RIGHT_MOUSE_BUTTON) { mouseState.buttons.right = true; }
            } else {
                if ((val & SCREEN_RIGHT_MOUSE_BUTTON) == 0) { mouseState.buttons.right = false; }
            }
            if ((mouse_buttons & SCREEN_MIDDLE_MOUSE_BUTTON) == 0) {
                ...
                if ((val & SCREEN_MIDDLE_MOUSE_BUTTON) == 0) { mouseState.buttons.middle = false; }
            }
            mouse_buttons = val;
            ...
            if (val != 0) { camera.translate(glm::vec3(0.0f, 0.0f, (float)val * 0.005f)); }
            ...
            if ((pos[0] != 0) || (pos[1] != 0)) { handleMouseMove(pos[0], pos[1]); }
            updateOverlay();
            break;
        }
    }
...
void VulkanExampleBase::setupWindow() {
    ...
    if (screen_pipeline_set) { usage |= SCREEN_USAGE_OVERLAY; }
...
void VulkanExampleBase::createSynchronizationPrimitives() {
    ...
    for (auto &fence : waitFences) { VK_CHECK_RESULT(vkCreateFence(device, &fenceCreateInfo, nullptr, &fence)); }
...
void VulkanExampleBase::beginDynamicRendering(VkCommandBuffer cmdBuffer) {
    vks::tools::insertImageMemoryBarrier(cmdBuffer,
    ...
    VkRenderingAttachmentInfoKHR depthStencilAttachment{.sType = VK_STRUCTURE_TYPE_RENDERING_ATTACHMENT_INFO_KHR,
...
void VulkanExampleBase::windowResize() {
    ...
    if ((width > 0.0f) && (height > 0.0f)) {
        if (settings.overlay) { ui.resize(width, height); }
    }
    ...
    if ((width > 0.0f) && (height > 0.0f)) { camera.updateAspectRatio((float)width / (float)height); }
...
void VulkanExampleBase::handleMouseMove(int32_t x, int32_t y) {
    ...
    if (mouseState.buttons.right) { camera.translate(glm::vec3(-0.0f, 0.0f, dy * .005f)); }
    if (mouseState.buttons.middle) { camera.translate(glm::vec3(-dx * 0.005f, -dy * 0.005f, 0.0f)); }
    mouseState.position = glm::vec2((float)x, (float)y);
}
